...
<div class="pull-right">
    <ul class="pagination">
        <c:if test="${pageMaker.prev}">
            <li class="page-item"><a class="page-link" href="/itemSearch/list?pageNum=${pageMaker.startPage - 1}&color=${color}&itemName=${itemName}">Previous</a></li>
        </c:if>
        ...
        <c:forEach var="num" begin="${pageMaker.startPage}" end="${pageMaker.endPage}" varStatus="loop">
            <li class="page-item ${pageMaker.pageNum == num ? 'active' : ''}">
                <a class="page-link" href="/itemSearch/list?pageNum=${num}&color=${color}&itemName=${itemName}">${num}</a>
            </li>
        </c:forEach>

        <c:if test="${pageMaker.next}">
            <li class="page-item"><a class="page-link" href="/itemSearch/list?pageNum=${pageMaker.endPage + 1}&color=${color}&itemName=${itemName}">Next</a></li>
        </c:if>
    </ul>
</div>
<!-- /페이징처리 -->
